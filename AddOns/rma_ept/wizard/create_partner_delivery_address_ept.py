...
class CreatePartnerDeliveryAddress(models.TransientModel):
    ...
    def create_new_contact_partner(self):
        ...
        taken changes by hare mori on date 02/03/2020 Changes were marge the method to create a
        new delivery address from the helpdesk ticket .
        """
        id = self._context.get('record')
        if self._context.get('active_model') == 'crm.claim.ept':
            rma_tickte_id = self.env['crm.claim.ept'].browse(id)
        else:
            rma_tickte_id = self.env['helpdesk.ticket'].browse(id)
        ...
        if is_contact_person:
            rma_tickte_id.write({'rma_support_person_id':new_partner_id.id})
        else:
            rma_tickte_id.write({'partner_delivery_id':new_partner_id.id})
        return True
